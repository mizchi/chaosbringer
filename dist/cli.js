...
if (values["har-record"])
    har = { path: values["har-record"], mode: "record" };
if (values["har-replay"])
    har = { path: values["har-replay"], mode: "replay" };
...
function parseNumberFlag(flag, raw, opts) {
    if (raw === undefined)
        return undefined;
    const n = Number(raw);
...
function buildInvariants() {
    ...
    if (values["visual-baseline"]) {
        list.push(visualRegression({
            baselineDir: values["visual-baseline"],
            threshold: parseNumberFlag("--visual-threshold", values["visual-threshold"], {
                min: 0,
                max: 1,
            }),
            maxDiffPixels: parseNumberFlag("--visual-max-diff-pixels", values["visual-max-diff-pixels"], { min: 0, integer: true }),
            maxDiffRatio: parseNumberFlag("--visual-max-diff-ratio", values["visual-max-diff-ratio"], { min: 0, max: 1 }),
    ...
    try {
        ...
        if (wantsHeatmap) {
            ...
            if (values.heatmap && !isQuiet) {
                const top = parseNumberFlag("--heatmap-top", values["heatmap-top"], {
                    min: 0,
                    integer: true,
                }) ?? 20;
                console.log("");
