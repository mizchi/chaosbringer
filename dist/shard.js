 ...
 */
export function shardOwns(url, shardIndex, shardCount) {
    if (shardCount <= 1)
        return true;
...
export function mergeReports(reports) {
    ...
    const deadLinkKey = (u, s) => `${u}|${s ?? ""}`;
    const deadLinks = new Map();
    const spaIssues = new Map();
    let extractedLinks = 0;
    let clickedLinks = 0;
    for (const r of reports) {
        for (const p of r.pages) {
            if (!pageByUrl.has(p.url))
...
export function parseShardArg(value) {
    ...
    if (!m) {
        throw new Error(`chaosbringer: --shard must be in the form "<index>/<count>" (got ${JSON.stringify(value)})`);
    }
    ...
    if (shardIndex < 0 || shardIndex >= shardCount) {
        throw new Error(`chaosbringer: --shard index ${shardIndex} out of range [0, ${shardCount})`);
    }
    return { shardIndex, shardCount };
}
function parseShardCliArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--count" || a === "--shards") {
            const v = argv[++i];
            if (!v)
                throw new Error(`chaosbringer shard: ${a} requires a value`);
            count = Number(v);
            continue;
        }
        ...
        if (a === "--output" || a === "-o") {
            const v = argv[++i];
            if (!v)
                throw new Error("chaosbringer shard: --output requires a value");
            output = v;
            continue;
        }
...
function runShardWorker(args) {
    return new Promise((resolve) => {
        const child = spawn(args.nodeBin, [...args.execArgv, args.scriptPath, ...args.workerArgs], {
...
export async function runShardCli(argv) {
    ...
    try {
        ...
        for (const r of results) {
            try {
                ...
                console.error(`shard: could not read ${r.reportPath} — ${err.message}`);
            }
        }
        if (reports.length === 0) {
