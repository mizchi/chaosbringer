 ...
 */
export function flakeReport(reports) {
    ...
    const flakyClusters = [];
    for (const occ of keyMap.values()) {
        if (occ.runsWithCluster === runs)
            stableClusters.push(occ);
        else if (occ.runsWithCluster > 0)
            flakyClusters.push(occ);
    }
    ...
    reports.forEach((report, idx) => {
        for (const page of report.pages) {
            ...
            const isFailed = page.errors.length > 0 || page.status === "error" || page.status === "timeout";
...
export function formatFlakeReport(analysis) {
    const lines = [];
    ...
    for (const c of analysis.flakyClusters) {
        lines.push(`  [${c.type}] ${truncate(c.fingerprint, 70)}  runs=${c.runsWithCluster}/${analysis.runs}  counts=${c.perRunCounts.join(",")}`);
    }
    lines.push("");
...
export async function runFlakeCli(argv) {
