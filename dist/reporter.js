...
export function formatReport(report) {
    ...
    if (report.summary.avgMetrics) {
        ...
        const entries = [];
        if (m.ttfb > 0)
            entries.push(`  TTFB: ${m.ttfb.toFixed(0)}ms`);
        if (m.fcp > 0)
            entries.push(`  FCP: ${m.fcp.toFixed(0)}ms`);
        if (m.lcp > 0)
            entries.push(`  LCP: ${m.lcp.toFixed(0)}ms`);
        if (entries.length > 0) {
            lines.push("");
    ...
    if (report.diff) {
        ...
        lines.push("-".repeat(40));
        lines.push(`  New clusters: ${d.newClusters.length}   Resolved: ${d.resolvedClusters.length}   Unchanged: ${d.unchangedClusters.length}`);
        lines.push(`  New failing pages: ${d.newFailedPages.length}   Resolved: ${d.resolvedFailedPages.length}`);
...
export function formatCompactReport(report, strict = false) {
...
export function printReport(report, compact = false, strict = false) {
...
export function getExitCode(report, strict = false) {
    const opts = typeof strict === "boolean" ? { strict } : strict;
