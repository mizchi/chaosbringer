...
import { matchesAnyPattern, matchesSpaPattern as matchesSpaPatternPure, isExternalUrl as isExternalUrlPure, escapeSelector as escapeSelectorPure, summarizePages, normalizeUrl, } from "./filters.js";
...
import { TRACE_FORMAT_VERSION, actionToTraceEntry, groupTrace, readTrace, writeTrace, } from "./trace.js";
...
export class ChaosCrawler {
    ...
    constructor(options, events = {}) {
        ...
        this.options = { ...DEFAULT_OPTIONS, ...filteredOptions };
     ...
     */
    reclassifyRejections(errors, rejections, url) {
        for (const rejection of rejections) {
            if (this.shouldIgnoreError(rejection.message))
                continue;
            const existing = errors.find((e) => e.type === "exception" && e.message === rejection.message);
            if (existing) {
     ...
     */
    async runInvariants(phase, page, url, errors) {
        ...
        for (const inv of invariants) {
            ...
            if (inv.urlPattern) {
                ...
                if (re && !re.test(url))
                    continue;
                if (!re)
                    continue; // Invalid pattern — silently skip (already flagged by validateOptions).
            }
    ...
    async drainRejections(page) {
        try {
            return await page.evaluate(() => {
                // @ts-ignore
                const bag = (window.__chaosRejections || []);
                // @ts-ignore
    ...
    createRecoveryInfo(failedUrl, error) {
        return {
            failedUrl,
            error,
    ...
    async start() {
        ...
        const deviceDesc = this.options.device && devices[this.options.device]
     ...
     * wastes visit budget.
     */
    async seedQueueFromSitemap(source) {
        let urls;
        try {
     ...
     */
    async applyNetworkProfile(page, profile) {
        try {
    ...
    async setupNavigationBlocking(page) {
        ...
        await page.route("**/*", async (route) => {
            ...
            for (const compiled of rules) {
                if (!compiled.pattern.test(url))
                    continue;
                if (compiled.methods && !compiled.methods.includes(method))
                    continue;
                compiled.matched++;
    ...
    async crawlPage(entry) {
        ...
        try {
            ...
            if (this.options.enableRecovery &&
                ...
                const recovery = this.createRecoveryInfo(url, `HTTP ${result.statusCode}`);
    ...
    async crawlPageWithExistingPage(page, url) {
        const errors = [];
        const warnings = [];
        ...
        page.on("pageerror", (err) => {
            if (!collecting)
                return;
            if (this.shouldIgnoreError(err.message))
                return;
            const error = {
                type: "exception",
                message: err.message,
        ...
        page.on("requestfailed", (request) => {
            ...
            if (this.shouldIgnoreError(requestUrl))
                return;
        ...
        try {
            ...
            result = {
                ...
                links,
                screenshot,
                blockedNavigations: this.blockedExternalCount > originalBlockedCount ? blockedNavigations : undefined,
            };
        }
        catch (err) {
            const loadTime = Date.now() - startTime;
     ...
     * running browser.
     */
    enforcePerformanceBudget(metrics, url, errors) {
    ...
    async extractLinks(page) {
        try {
            const links = await page.evaluate(() => {
                ...
                for (const link of Array.from(document.querySelectorAll('link[rel~="canonical"][href], link[rel~="alternate"][href]'))) {
     ...
     */
    async getWeightedActionTargets(page) {
        const targets = [];
        try {
            ...
            for (const t of domTargets) {
                ...
                if (t.tag === "a") {
                    ...
                    if (t.isNavLink)
                        weight *= 1.5; // Boost navigation links
     ...
     */
    async performWeightedActions(page, url) {
     ...
     */
    async performReplayActions(page, url, actions) {
    ...
    async performActionOnTarget(page, target, url) {
        const timestamp = Date.now();
        try {
    ...
    buildReproCommand() {
        const parts = ["chaosbringer", "--url", shellQuote(this.options.baseUrl)];
...
export function validateOptions(options) {
    ...
    try {
        ...
        throw new Error(`chaosbringer: "baseUrl" must be an absolute URL (got ${JSON.stringify(options.baseUrl)})`);
    }
    const requirePositive = (name, value, min) => {
        if (value === undefined)
            return;
        if (!Number.isFinite(value) || !Number.isInteger(value) || value < min) {
            throw new Error(`chaosbringer: "${name}" must be an integer >= ${min} (got ${JSON.stringify(value)})`);
        }
    };
    ...
    if (options.shardIndex !== undefined || options.shardCount !== undefined) {
        if (options.shardCount === undefined || options.shardIndex === undefined) {
            throw new Error(`chaosbringer: "shardIndex" and "shardCount" must be set together`);
        }
        if (!Number.isInteger(options.shardCount) ||
            options.shardCount < 1) {
            throw new Error(`chaosbringer: "shardCount" must be an integer >= 1 (got ${JSON.stringify(options.shardCount)})`);
        }
        if (!Number.isInteger(options.shardIndex) ||
            options.shardIndex < 0 ||
            options.shardIndex >= options.shardCount) {
            throw new Error(`chaosbringer: "shardIndex" must be an integer in [0, ${options.shardCount}) (got ${JSON.stringify(options.shardIndex)})`);
        }
    }
    if (options.seed !== undefined) {
        if (!Number.isFinite(options.seed) || !Number.isInteger(options.seed) || options.seed < 0) {
            throw new Error(`chaosbringer: "seed" must be a non-negative integer (got ${JSON.stringify(options.seed)})`);
        }
    }
    const assertRegexString = (label, pattern) => {
    ...
    const assertMatcher = (label, m) => {
        if (m === undefined)
            return;
        if (m instanceof RegExp)
            return; // Already compiled, always valid.
        assertRegexString(label, m);
    };
    for (const p of options.excludePatterns ?? [])
        assertRegexString(`excludePatterns entry`, p);
    for (const p of options.ignoreErrorPatterns ?? [])
        assertRegexString(`ignoreErrorPatterns entry`, p);
    for (const p of options.spaPatterns ?? [])
        assertRegexString(`spaPatterns entry`, p);
    for (const rule of options.faultInjection ?? []) {
        ...
        if (rule.probability !== undefined) {
            ...
            if (!Number.isFinite(p) || p < 0 || p > 1) {
                throw new Error(`chaosbringer: ${label} probability must be in [0, 1] (got ${JSON.stringify(p)})`);
            }
        }
    }
    ...
    if (options.har) {
        ...
        if (mode !== "record" && mode !== "replay") {
            throw new Error(`chaosbringer: "har.mode" must be "record" or "replay" (got ${JSON.stringify(mode)})`);
        }
    }
    if (options.storageState !== undefined) {
        if (typeof options.storageState !== "string" || options.storageState.length === 0) {
            throw new Error(`chaosbringer: "storageState" must be a non-empty path string (got ${JSON.stringify(options.storageState)})`);
        }
    }
    const assertNonEmptyStringOpt = (name, v) => {
        if (v === undefined)
            return;
    ...
    if (options.device !== undefined) {
        if (typeof options.device !== "string" || options.device.length === 0) {
            throw new Error(`chaosbringer: "device" must be a non-empty Playwright device name (got ${JSON.stringify(options.device)})`);
        }
        if (!devices[options.device]) {
            throw new Error(`chaosbringer: "device" ${JSON.stringify(options.device)} is not a known Playwright device descriptor`);
        }
    }
    if (options.network !== undefined) {
        const allowed = new Set(NETWORK_PROFILES);
        if (typeof options.network !== "string" || !allowed.has(options.network)) {
            throw new Error(`chaosbringer: "network" must be one of ${NETWORK_PROFILES.join(", ")} (got ${JSON.stringify(options.network)})`);
        }
    }
    if (options.performanceBudget !== undefined) {
        const budget = options.performanceBudget;
        if (budget === null || typeof budget !== "object" || Array.isArray(budget)) {
            throw new Error(`chaosbringer: "performanceBudget" must be an object (got ${JSON.stringify(budget)})`);
        }
        const allowed = new Set(PERF_BUDGET_KEYS);
        for (const key of Object.keys(budget)) {
            if (!allowed.has(key)) {
                throw new Error(`chaosbringer: "performanceBudget.${key}" is not a known metric (allowed: ${PERF_BUDGET_KEYS.join(", ")})`);
            }
            const val = budget[key];
            if (typeof val !== "number" || !Number.isFinite(val) || val <= 0) {
                throw new Error(`chaosbringer: "performanceBudget.${key}" must be a positive number of ms (got ${JSON.stringify(val)})`);
            }
        }
    }
}
...
function compileFaultRules(rules) {
...
async function applyFault(route, fault) {
    switch (fault.kind) {
        case "abort":
        ...
        case "status": {
            ...
            const body = fault.body !== undefined ? fault.body : JSON.stringify({ error: fault.status });
            await route.fulfill({
                status: fault.status,
                body,
