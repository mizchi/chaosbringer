...
`emit-budgets`, `gate` and `regress` exit **2**, not 1, when their inputs were crawled under different [settle modes](#settling-between-steps---settle), and compare nothing; `--allow-settle-mismatch` compares anyway, with a warning. See [Settle modes and comparisons](#settle-modes-and-comparisons).
