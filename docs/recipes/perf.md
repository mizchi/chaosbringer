...
`regress` needs no such adjustment because of its absolute floors. Three of them were raised for crawls while taking these numbers (`chaosbringer perf regress` passes them as floor overrides; `lightbringer-regress` keeps its own defaults of 5 ms, 16 ms and 2 frames): `durationMs` from 5 to 17 ms, because an action span ends on its settle wait and so moves in frame-sized steps (a clean PR run saw a 51.4 → 60.7 ms click median); `longestFrameMs` from 16 to 17 ms, because frame durations come in whole 16.7 ms vsync intervals and a single late frame (16.8 → 33.3) failed a clean batch; and `droppedFrames` from 2 to 4, because headless Chromium on a shared CPU dropped 2–4 frames during a plain load with no app work (median 0 → 2 on a clean batch). The main-thread metrics (`scriptMs` floor 2 ms, `blockingMs` 5 ms) are the precise signal; the frame metrics catch jank of ~70 ms and up.
...
{ "key": "/item/:id :: load", "fault": "api-delay",
  "faulted": { "n": 4, "durationMs": 452, "effectiveMs": 452, "blockingMs": 0, "requestCount": 3 },
  "clean":   { "n": 4, "durationMs": 118, "effectiveMs": 118, "blockingMs": 0, "requestCount": 3 },
  "delta":   { "durationMs": 334, "effectiveMs": 334, "blockingMs": 0, "requestCount": 0 } }
```
...
- **`effectiveMs` is how long until the step's own work finished**: per span, `max(durationMs, network.settledMs)`, where lightbringer's `settledMs` is when the last request that *started* in the span finished (a failed or aborted one counts at the time it failed). It differs from `durationMs` when a step fires a request and does not wait for it. That is the normal case for a click under the default `networkidle` settle: the post-click wait does not wait for the request (see [settling](#settling-between-steps---settle)), so the click's span closes a few ms after its fetch starts, delayed or not, and `delta.durationMs` for a latency fault on that fetch is click-to-click noise. `delta.effectiveMs` carries the delay. In the 2026-09 evaluation's XHR button with a 300 ms delay, `durationMs` read −11 to −29 ms and `effectiveMs` +273 to +290 ms (+281 ms under `--settle adaptive`, where the span waits for the request itself). Load spans are not affected: their settle waits for their requests, so both numbers agree.
- Read `delta.effectiveMs` for "what did the fault cost this step", and `delta.durationMs` for "how much longer did the step itself block the crawl". A large `effectiveMs` delta with a `durationMs` delta near 0 is a fault the page absorbed in the background: nothing waited on it, but the step's result showed up that much later.
- So that a click's request is over before its span is reported, the page's report is built only once no request an action span started is still running, waiting at most 2 s (the same cap as an action's adaptive settle). A request still running after that (a long poll, a hang fault) leaves `network.settledUnfinished: true` on its span and is not counted, so `effectiveMs` is then a lower bound.
- `interactionMs` appears on each side only over spans that had an interaction, and in `delta` only when both sides did.
- Sorted by `delta.effectiveMs`, largest first; the reporter prints the top five as "Degradation under faults", leading with the `effectiveMs` delta and the two sides' `effectiveMs` medians, and adding `span ±Nms` (the `durationMs` delta) when it differs. A `requestCount` delta is the retry storm the `amplification` oracle counts, priced in time.
...
- A candidate's weight is the mean observed cost of its perfKey, the larger of `blockingMs` and `interaction.maxDurationMs`, bucketed (below), plus a 1 ms floor. It is the larger, not the sum: a busy handler shows up in both, and its `interaction` often arrives only after the next step read the span (see above), so a sum read the same 200 ms click as 200 in one run and 400 in the next. Costs come from the steps of this crawl and, with `prior`, from every action span in an earlier report. `durationMs` is not part of the cost: it includes settle and network time, so a step that is slow only because its request is slow would outrank one that freezes the page.
...
- **Reproducibility.** For a seed, the pick sequence is a function of the candidates each step sees and of each key's bucketed mean cost. Every step draws exactly two values from `step.rng` (the pick roll, then the epsilon roll) whichever branch picks, so a step that goes a different way does not shift the rolls of every later step. The weights are built in the candidates' page order, and each cost is rounded to a whole millisecond before it is summed, so the order measurements arrived in does not matter. A mean below 42 ms (`PERF_SEEKING_COST_THRESHOLD_MS`) weighs the floor. Above that, means fall into log-scale buckets 1.5x wide (`PERF_SEEKING_COST_BUCKET_RATIO`, edges at 42, 63, 94.5, 142, 213, 319, 478, 718 ms and so on), and every mean in a bucket gives the same weight (`perfSeekingBucketCost`). Timing noise grows with the cost, so 162 vs 177 ms or 200 vs 208 ms land in the same bucket, while 80 vs 200 ms stay about 2.2x apart in weight. On a 10-button page with 200 ms and 80 ms handlers, 3 runs of each of 10 seeds made the same 30 picks every time. The limit: a mean that sits on a bucket edge, or a step whose cost really changed between runs (a GC pause, a slower machine, a 30% regression), still changes that pick and everything after it. A `prior` report counts as part of the input, so the same seed with a different prior gives a different sequence. Sequences recorded before this change do not replay: the draws, the cost and the buckets all changed. (`PERF_SEEKING_COST_BUCKET_MS`, the old 5 ms width, is still exported but no longer used.)
...
- **The HTTP cache is on, except where every request is routed.** Pages of one crawl share the browser cache, as a real repeat visit would: after the first page, a cacheable asset is served from cache (`perfPage.network.fromCacheCount`) and a site with no caching headers shows up as repeated downloads. External-navigation blocking (on by default) does not touch the cache — it pauses document requests only, over CDP. Fault injection (`faultInjection`), `traceparent` and HAR replay route *every* request through a Playwright route, and Playwright disables the cache on any page with a route, so those runs measure every page cold. `httpCache: false` (`--no-http-cache`) disables it everywhere, for comparing against baselines recorded before the cache was on; a perf baseline recorded cold is not comparable to a warm run.
