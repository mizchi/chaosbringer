...
State2 ==
  log = <<[kind |-> "start", op |-> "-"], [kind |-> "fulfil", op |-> "cart"]>>
    /\ opState
      = SetAsFun({ <<"cart", "fulfilled">>, <<"shipping", "pending">> })
    /\ ui = "loading"
    /\ unhandled = FALSE
...
State3 ==
  log
      = <<
        [kind |-> "start", op |-> "-"], [kind |-> "fulfil", op |-> "cart"], [kind |->
            "fulfil",
          op |-> "shipping"]
      >>
    /\ opState
...
================================================================================
(* Created by Apalache on Thu Aug 20 11:05:53 UTC 2026 *)
