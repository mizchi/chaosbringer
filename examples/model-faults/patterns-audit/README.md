...
six [patterns](../patterns/) built on it, which is a different question: not
...
```bash
npx tsx patterns-audit/audit.mts          # every finding + both refutations (~2min)
npx tsx patterns-audit/audit.mts F2       # one of them
npx tsx patterns-audit/model-vacuity.mts  # F6, no browser (~19s)
...
| F6 | all models | 13 of 22 `contract-forbids-*` targets could only be answered one way | **closed as tooling** — [`patterns/vacuity.mjs`](../patterns/vacuity.mjs) annotates each one |
