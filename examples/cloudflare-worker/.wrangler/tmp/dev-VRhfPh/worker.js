...
// .wrangler/tmp/bundle-5IXjpf/strip-cf-connecting-ip-header.js
...
// ../../packages/server-faults/dist/server-faults.js
...
__name(pickLatencyMs, "pickLatencyMs");
function compileStatelessPattern(p) {
  if (!p)
    return null;
  if (typeof p === "string")
    return new RegExp(p);
...
__name(compileStatelessPattern, "compileStatelessPattern");
...
__name(extractTraceId, "extractTraceId");
var sleep = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "sleep");
var DEFAULT_METADATA_PREFIX = "x-chaos-fault";
var FAULT_HEADER_KEY_SUFFIX = {
  kind: "kind",
  path: "path",
  method: "method",
...
function attrsToHeaderEntries(attrs, prefix) {
  const out = [];
  for (const k of Object.keys(FAULT_HEADER_KEY_SUFFIX)) {
    const v = attrs[k];
    if (v === void 0)
      continue;
...
__name(attrsToHeaderEntries, "attrsToHeaderEntries");
function resolveMetadataPrefix(opt) {
  if (!opt)
    return null;
  if (opt === true)
    return DEFAULT_METADATA_PREFIX;
  return opt.prefix ?? DEFAULT_METADATA_PREFIX;
}
__name(resolveMetadataPrefix, "resolveMetadataPrefix");
function mintSyntheticFault(status, path, method, traceId, cfg, metadataPrefix) {
  const attrs = {
    kind: "5xx",
    path,
    method,
  ...
  const response = Response.json({ error: "chaos: synthetic 5xx", path, status }, { status, headers });
  return { kind: "synthetic", response, attrs };
}
__name(mintSyntheticFault, "mintSyntheticFault");
function serverFaults(cfg) {
  ...
  const rng = cfg.seed !== void 0 ? mulberry32(cfg.seed) : { next: () => Math.random() };
  return {
    async maybeInject(req) {
      if (bypassHeader && req.headers.has(bypassHeader))
        return null;
      const url = new URL(req.url);
      if (exemptPattern && exemptPattern.test(url.pathname))
        return null;
      ...
      if (rA > 0 && rng.next() < rA) {
        const abortStyle = cfg.abortStyle ?? "hangup";
        const attrsAbort = {
          kind: "abort",
          path: url.pathname,
          method,
      ...
      if (rS > 0 && rng.next() < rS) {
        ...
        if (chunkSize !== void 0)
          attrsSlow.chunkSize = chunkSize;
        if (traceId !== void 0)
          attrsSlow.traceId = traceId;
        Object.freeze(attrsSlow);
        cfg.observer?.onFault?.("slowStream", attrsSlow);
        return chunkSize !== void 0 ? { kind: "slowStream", attrs: attrsSlow, chunkDelayMs, chunkSize } : { kind: "slowStream", attrs: attrsSlow, chunkDelayMs };
      }
...
__name(serverFaults, "serverFaults");

// ../../packages/server-faults/dist/stream-transforms.js
var sleep2 = /* @__PURE__ */ __name((ms) => new Promise((resolve) => setTimeout(resolve, ms)), "sleep");
function truncateStream(source, afterBytes) {
  if (!source) {
...
__name(truncateStream, "truncateStream");
function slowStream(source, opts) {
  if (!source) {
    return new ReadableStream({
      start(controller) {
        controller.close();
  ...
  return new ReadableStream({
    async pull(controller) {
      if (chunkSize === void 0) {
        ...
        await sleep2(opts.chunkDelayMs);
        controller.enqueue(value);
        return;
      }
      while (true) {
...
__name(slowStream, "slowStream");

// ../../packages/server-faults/dist/adapters/hono.js
var ServerFaultsAbortError = class extends Error {
  abortStyle;
  constructor(abortStyle) {
    super("server-faults: synthetic abort");
    this.name = "ServerFaultsAbortError";
    this.abortStyle = abortStyle;
  }
};
__name(ServerFaultsAbortError, "ServerFaultsAbortError");
function honoMiddleware(cfg) {
...
__name(honoMiddleware, "honoMiddleware");
function isFullResponse(r) {
  return !!r && "body" in r;
}
__name(isFullResponse, "isFullResponse");
...
function envKey(env2) {
  return [
    env2.DEPLOY_ENV ?? "",
    env2.CHAOS_5XX_RATE ?? "",
    env2.CHAOS_LATENCY_RATE ?? "",
    env2.CHAOS_LATENCY_MS ?? "",
    env2.CHAOS_SEED ?? ""
  ].join("|");
}
__name(envKey, "envKey");
function createApp(env2) {
  const key = envKey(env2);
  if (cachedApp && cachedEnvKey === key)
    return cachedApp;
  cachedEnvKey = key;
  cachedApp = buildApp(env2);
  return cachedApp;
}
__name(createApp, "createApp");
function buildApp(env2) {
  const app = new Hono2();
  const r5 = Number(env2.CHAOS_5XX_RATE ?? "0");
  const rL = Number(env2.CHAOS_LATENCY_RATE ?? "0");
  if (env2.DEPLOY_ENV === "dev" && (r5 > 0 || rL > 0)) {
    app.use(
      "/api/*",
      honoMiddleware({
        status5xxRate: r5,
        latencyRate: rL,
        latencyMs: Number(env2.CHAOS_LATENCY_MS ?? "0"),
        seed: env2.CHAOS_SEED ? Number(env2.CHAOS_SEED) : void 0,
        metadataHeader: true,
...
<p><a href="/">back</a></p>`);
  ...
  app.get(
    "/about",
    (c) => c.html(`<!doctype html><title>About</title><h1>About</h1>
...
var worker_default = {
  fetch(request, env2, ctx) {
    return createApp(env2).fetch(request, env2, ctx);
  }
};
...
// .wrangler/tmp/bundle-5IXjpf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-5IXjpf/middleware-loader.entry.ts
