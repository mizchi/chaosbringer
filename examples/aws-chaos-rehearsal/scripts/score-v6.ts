...
import { checkoutReceiptsStalled } from "../../../packages/aws-faults/src/wheel/scenarios/checkout-receipts-stalled.ts";

const transcript = readFileSync("/tmp/wom-eval6/transcript.txt", "utf8");
const toolUses: ToolUseRecord[] = readFileSync("/tmp/wom-eval6/tool-uses.jsonl", "utf8").trim().split("\n").map((l) => JSON.parse(l));
const journalContents = [readFileSync("/tmp/wom-eval6/journal.md", "utf8")];
...
const scenario = checkoutReceiptsStalled({
...
const drillReport: DrillReport = {
  ...
  injectedByPhase: [{ label: "peak", samples: [{ ok: false, latencyMs: 6000, errorRate: 0.93 }] }],
  recovery: Array.from({ length: 20 }, () => ({ ok: true, latencyMs: 30, errorRate: 0 })),
  durationMs: 95_000, recovered: true,
};
