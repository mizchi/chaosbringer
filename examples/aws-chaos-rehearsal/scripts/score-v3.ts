...
const transcript = readFileSync("/tmp/wom-eval3/transcript.txt", "utf8");
const toolUses: ToolUseRecord[] = readFileSync("/tmp/wom-eval3/tool-uses.jsonl", "utf8").trim().split("\n").map((l) => JSON.parse(l));
const journalContents = [readFileSync("/tmp/wom-eval3/journal.md", "utf8")];
...
const drillReport: DrillReport = {
  ...
  durationMs: 180_000, recovered: true,
};
